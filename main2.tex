...
To address the limitations of existing multimodal glaucoma analysis approaches, this work introduces several methodological contributions and novel design aspects within a unified multimodal transformer framework. The main contribution of this study is not the isolated use of existing transformer components, but the design and evaluation of a data-efficient, task-aligned multimodal learning framework for paired fundus-OCT glaucoma analysis under the limited-data setting of the GAMMA dataset. Specifically, this work makes the following contributions:

\begin{itemize}
    \item We formulate two clinically relevant tasks from the GAMMA dataset: binary glaucoma screening, obtained by merging early and mid/advanced glaucoma as the positive class, and three-class glaucoma severity grading. This enables evaluation of both referral-level detection and ordinal disease staging within the same paired fundus-OCT cohort.

    \item We propose and compare early, deep, and late multimodal fusion strategies under a unified experimental protocol. This controlled comparison allows the effect of fusion depth to be studied independently from differences in preprocessing, optimization, and evaluation settings.

    \item We introduce an OCT multiple instance learning (OCT-MIL) strategy to aggregate diagnostic evidence from a selected subset of OCT B-scans. Rather than processing the entire 256-slice OCT volume, which is computationally expensive and prone to overfitting with only 300 labelled paired samples, the number of OCT slices is treated as a hyperparameter during Optuna-based model selection.

    \item We implement optional fusion enhancement modules, including FiLM-based feature modulation, cross-modal attention with gating, and temperature-weighted confidence-aware decision fusion. These components can be selectively activated depending on the diagnostic task, allowing task-aligned architectural flexibility while maintaining a unified core design.

    \item We evaluate multiple transformer-family backbones, including Vision Transformer, DeiT, Swin Transformer, and CoAtNet variants, to examine how global self-attention, hierarchical local attention, and convolution--attention hybrid inductive biases affect multimodal glaucoma representation learning.

    \item We generate Grad-CAM-based explainability visualizations for both fundus and OCT modalities, highlighting clinically relevant regions for model predictions, thereby enhancing interpretability and clinical trust in the model's decision-making process.
\end{itemize}
...
Beyond glaucoma-specific multimodal modelling, generic image-fusion methods provide useful methodological context for understanding fusion design. DeepFuse introduced an unsupervised deep learning framework for multi-exposure image fusion, while U2Fusion proposed a unified unsupervised network for multi-modal, multi-exposure, and multi-focus fusion by preserving complementary information from source images without requiring ground-truth fused targets~\cite{ram2017deepfuse,xu2020u2fusion}. More recent multimodal medical image fusion methods, such as ECFusion, further incorporate edge enhancement and cross-scale transformer mechanisms to preserve structural detail and multi-scale contextual information~\cite{luo2025novel}. However, these frameworks are not directly equivalent to the supervised fundus-OCT glaucoma classification problem considered in this study, because their primary objective is fused-image reconstruction or visual information preservation rather than diagnostic prediction. Applying such methods to GAMMA would require an additional downstream glaucoma classifier, making performance dependent on both the image-fusion stage and the classifier. Therefore, generic image-fusion frameworks are discussed here as methodological context, whereas the empirical comparison in this study focuses on diagnostic multimodal models trained under the same paired fundus-OCT classification protocol.
...
Overall, prior work highlights four consistent gaps. First, many glaucoma AI studies rely on a single modality and therefore underuse complementary retinal information from fundus photography and OCT. Second, existing multimodal glaucoma studies typically employ a fixed fusion design, limiting insight into how the timing of multimodal interaction affects diagnostic behaviour. Third, most GAMMA-based studies focus primarily on multiclass grading, whereas clinical deployment also requires referral-oriented binary screening. Fourth, paired fundus-OCT datasets remain small; therefore, multimodal models must be explicitly designed for data-efficient learning rather than assuming large-scale training conditions. These limitations motivate the present study, which systematically evaluates Early, Deep, and Late Fusion strategies within a unified transformer-based multimodal pipeline for both glaucoma screening and glaucoma grading, while incorporating OCT-MIL, cross-validation, calibration, bootstrap uncertainty estimation, and explainability analysis.
